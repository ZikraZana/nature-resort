<?php
/**
 * Invoice Booking — Kincay Mania Hotel & Resort
 * Halaman cetak invoice (print-friendly) dari data database.
 */
...
$stmt = db()->prepare(
    'SELECT b.*, k.nama AS kamar_nama, k.tipe AS kamar_tipe, k.harga_per_malam,
            u.nama AS tamu_nama, u.email AS tamu_email, u.no_hp AS tamu_telepon
     FROM booking b
...
if (!$booking || !in_array($booking['status'], ['dikonfirmasi', 'checkin', 'selesai', 'menunggu_refund', 'refund_selesai'])) {
    set_flash('danger', 'Invoice tidak tersedia untuk booking ini.');
...
// Query info resort
$stmtBank = db()->query('SELECT * FROM pengaturan_sistem LIMIT 1');
$setting = $stmtBank->fetch();
...
<body>
    <div class="no-print" style="text-align: center; padding: 20px; background: #F4F0E5;">
        ...
        <a href="<?= BASE_URL ?>/tamu/booking_detail.php?id=<?= $bookingId ?>" class="back-btn">← Kembali</a>
    </div>
    ...
    <div class="invoice-container">
        ...
        <div class="header">
            <div class="brand">
                <h1>🏔️ Kincay Mania</h1>
                <p>Hotel & Resort — Nature Resort Kerinci</p>
                <p style="margin-top: 8px">Kerinci, Jambi, Sumatera</p>
            </div>
            <div class="invoice-meta">
                <h2>INVOICE</h2>
                <p><strong>#INV-<?= str_pad($booking['id'], 4, '0', STR_PAD_LEFT) ?></strong></p>
                <p>Tanggal: <?= date('d M Y', strtotime($booking['created_at'])) ?></p>
                <p style="margin-top: 8px"><span class="status-badge status-success"><?= ucfirst(str_replace('_', ' ', $booking['status'])) ?></span></p>
            </div>
        </div>
        ...
        <div class="info-grid">
            <div class="info-box">
                ...
                <p><strong><?= e($booking['tamu_nama']) ?></strong></p>
                <p><?= e($booking['tamu_email']) ?></p>
                <p><?= e($booking['tamu_telepon'] ?: '-') ?></p>
            </div>
            <div class="info-box" style="text-align: right;">
                <h3>Detail Menginap</h3>
                <p>Check-in: <strong><?= date('d M Y', strtotime($booking['tanggal_checkin'])) ?></strong></p>
                <p>Check-out: <strong><?= date('d M Y', strtotime($booking['tanggal_checkout'])) ?></strong></p>
                <p>Durasi: <strong><?= $jumlahMalam ?> malam</strong></p>
                <p>Tamu: <strong><?= $booking['jumlah_tamu'] ?> orang</strong></p>
            </div>
        </div>
        ...
        <table>
            ...
            <tbody>
                <tr>
                    <td><strong><?= e($booking['kamar_nama']) ?></strong> (<?= e($booking['kamar_tipe']) ?>)</td>
                    <td><?= $jumlahMalam ?> malam</td>
                    <td><?= format_rupiah($booking['harga_per_malam']) ?></td>
                    <td><?= format_rupiah($hargaKamar) ?></td>
                </tr>
                <?php
                $totalPaket = 0;
                ...
                ?>
                <tr>
                    <td><?= e($pw['paket_nama']) ?> <span style="color:#6B705C; font-size:12px">(<?= date('d M Y', strtotime($pw['tanggal'])) ?>)</span></td>
                    <td><?= $pw['jumlah_peserta'] ?> peserta</td>
                    <td><?= format_rupiah($pw['paket_harga']) ?></td>
                    <td><?= format_rupiah($pw['subtotal']) ?></td>
                </tr>
                <?php endforeach; ?>
            </tbody>
            <tfoot>
                <tr>
                    <td colspan="3">Subtotal Kamar</td>
                    <td><?= format_rupiah($hargaKamar) ?></td>
                </tr>
                <?php if ($totalPaket > 0): ?>
                <tr>
                    <td colspan="3">Subtotal Paket Wisata</td>
                    <td><?= format_rupiah($totalPaket) ?></td>
                </tr>
                <?php endif; ?>
                <tr>
                    <td colspan="3"><strong>TOTAL</strong></td>
                    <td><strong><?= format_rupiah($booking['total_harga']) ?></strong></td>
                </tr>
            </tfoot>
        </table>
        ...
        <!-- Footer -->
        <div class="footer">
            <p>Terima kasih telah menginap di Kincay Mania Hotel & Resort!</p>
            <p style="margin-top: 4px">Dokumen ini merupakan bukti booking resmi yang sah. Harap dibawa saat check-in.</p>
        </div>
    </div>
</body>
</html>

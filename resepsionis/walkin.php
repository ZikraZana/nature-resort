    ...
    if (empty($errors)) {
        ...
        try {
            ...
            if ($status === 'dikonfirmasi') {
                set_flash('success', 'Booking walk-in #BK-' . str_pad($bookingId, 4, '0', STR_PAD_LEFT) . ' berhasil, langsung dikonfirmasi.');
                header('Location: ' . BASE_URL . '/resepsionis/walkin.php');
            } else {
                // Transfer: lanjut ke halaman upload bukti transfer
                header('Location: ' . BASE_URL . '/resepsionis/walkin_upload_bukti.php?booking_id=' . $bookingId);
            }
            exit;
        } catch (\Exception $e) {
            $pdo->rollBack();
...
// Query booking walk-in yang belum upload bukti transfer (supaya tidak nyangkut/hilang dari pandangan)
$stmt = db()->prepare(
    "SELECT b.id, b.nama_tamu, b.kontak_tamu, b.total_harga, b.created_at,
            k.nama AS kamar_nama
     FROM booking b
     JOIN kamar k ON k.id = b.kamar_id
     WHERE b.status = 'menunggu_pembayaran' AND b.dibuat_oleh IS NOT NULL AND b.user_id IS NULL
     ORDER BY b.created_at DESC"
);
$stmt->execute();
$menungguBukti = $stmt->fetchAll();
...
<section class="pt-24 pb-16 bg-cream min-h-screen">
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <?php if (!empty($menungguBukti)): ?>
            <div class="mb-6 bg-warning-light border border-warning/30 rounded-2xl p-5">
                <h3 class="font-semibold text-dark mb-3 flex items-center gap-2">
                    <svg class="w-5 h-5 text-warning" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                    </svg>
                    Booking Walk-in Menunggu Bukti Transfer
                </h3>
                <p class="text-sm text-earth mb-3">Booking berikut sudah dibuat tapi bukti transfer belum diupload. Klik untuk lanjutkan.</p>
                <div class="space-y-2">
                    <?php foreach ($menungguBukti as $b): ?>
                        <a href="<?= BASE_URL ?>/resepsionis/walkin_upload_bukti.php?booking_id=<?= $b['id'] ?>"
                            class="flex items-center justify-between bg-white rounded-xl p-3 hover:shadow-sm transition-shadow">
                            <div>
                                <p class="font-medium text-dark text-sm">#BK-<?= str_pad($b['id'], 4, '0', STR_PAD_LEFT) ?> — <?= e($b['nama_tamu']) ?></p>
                                <p class="text-xs text-earth"><?= e($b['kamar_nama']) ?> · <?= format_rupiah($b['total_harga']) ?></p>
                            </div>
                            <span class="text-primary text-sm font-medium">Upload Bukti →</span>
                        </a>
                    <?php endforeach; ?>
                </div>
            </div>
        ...
        <form method="POST" action="<?= BASE_URL ?>/resepsionis/walkin.php" class="space-y-6">
            ...
            <div class="bg-white rounded-2xl p-6 shadow-sm">
                <h2 class="font-semibold text-dark mb-4 flex items-center gap-2"><svg class="w-5 h-5 text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <label class="cursor-pointer"><input type="radio" name="metode_bayar" value="tunai" class="peer hidden" <?= ($_POST['metode_bayar'] ?? 'tunai') === 'tunai' ? 'checked' : '' ?>>
                        <div class="border-2 border-cream-darker rounded-xl p-4 text-center transition-all peer-checked:border-primary peer-checked:bg-primary/5">
                            <svg class="w-8 h-8 mx-auto mb-2 text-earth" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2zm7-5a2 2 0 11-4 0 2 2 0 014 0z" />
                            </svg>
                            <p class="font-medium text-dark text-sm">Tunai</p>
                            <p class="text-xs text-earth">Langsung dikonfirmasi</p>
                        </div>
                    </label>
                    <label class="cursor-pointer"><input type="radio" name="metode_bayar" value="transfer" class="peer hidden" <?= ($_POST['metode_bayar'] ?? '') === 'transfer' ? 'checked' : '' ?>>
                        <div class="border-2 border-cream-darker rounded-xl p-4 text-center transition-all peer-checked:border-primary peer-checked:bg-primary/5">
                            ...
                            <p class="font-medium text-dark text-sm">Transfer</p>
                            <p class="text-xs text-earth">Masuk alur verifikasi</p>
                        </div>
                    </label>
                </div>
            </div>
        ...
        <script>
            (function() {
                const kamarPrices = {
                    <?php foreach ($kamarList as $k): ?><?= $k['id'] ?>: {
                        nama: <?= json_encode($k['nama']) ?>,
                        harga: <?= $k['harga_per_malam'] ?>
                    },
                <?php endforeach; ?>
                };
